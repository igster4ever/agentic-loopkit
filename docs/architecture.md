...
| `SelfHarnessExecutor` | `loops/self_harness.py` | `OutcomeExecutor` subclass; wires `FailurePatternAgent` → `SkillOptExecutor` → `AgentTestHarness.regression_gate()` → emit `harness.edit_accepted/rejected`; evaluate() is deterministic — no LLM. **P71 (2026-07-30):** first-iteration Phase-B adaptation (MemoHarness arXiv:2607.14159 §2.6) — deterministic `_ADAPTATION_MAP` lookup nudges bounded `train_fraction` per case from own-mechanism `FailureSignature` clusters, emits `harness.harness_adapted`; distinct from `SkillOptExecutor`'s Phase-A skill-text search | ✅ Built (2026-06-16; P71 adaptation 2026-07-30) |
