...
class AgentBase(ABC):
    ...
    async def save_state(self, state: AgentState) -> None:
        ...
        Base implementation writes semantic facts to _memory_store when set.
        Override to customise episodic or procedural persistence.
        """
    ...
    async def load_state(self) -> AgentState:
        ...
        Base implementation reads semantic facts from _memory_store when set.
        Override to customise episodic or procedural loading.
        """
        semantic: dict[str, Any] = {}
        if self._memory_store is not None:
            records = await self._memory_store.list(agent_id=self.name)
            semantic = {r.key: r.value for r in records}
        return AgentState(episodic=[], semantic=semantic, procedural={})
